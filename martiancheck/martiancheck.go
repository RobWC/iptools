...
//Contains checks to see if provided IP is found within the defined martian IP Net
//returns true if matched and false if not matched
...
//AddMartianNet adds a new martian net to the list of martian nets
func (ipch *MartianChecker) AddMartianNet(net net.IPNet) {
	ipch.Martians = append(ipch.Martians, net)
}

//RemoveMartianNet removes a martian net if exists, if it does not exist then returns false
...
//RemoveAllMartianNet removes all martian IP nets that are defined
